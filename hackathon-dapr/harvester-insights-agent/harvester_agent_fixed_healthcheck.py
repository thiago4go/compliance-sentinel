    ...
    try:
        ...
        if DAPR_AGENTS_AVAILABLE and agent:
            try:
                ...
                logger.info(f"Using DurableAgent for analysis: {framework} - {company_name}")
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(f"http://localhost:{dapr_port}/v1.0/healthz", timeout=5.0)
                # Accept both 200 and 204 as success
                if response.status_code in [200, 204]:
                    dapr_available = True
                    logger.info(f"Dapr sidecar detected and available (status: {response.status_code})")
                else:
                    logger.warning(f"Dapr healthz returned status: {response.status_code}")
        except Exception as e:
        ...
        if DAPR_AGENTS_AVAILABLE and dapr_available:
            ...
            if openai_key:
                try:
                    logger.info("Initializing DurableAgent with OpenAI...")
